...
export class ProjectTaskDeadlineCardComponent implements OnInit{
  ...
  generateMenu(subMenu: any[]) {
    this.menuItems = [
      {label: 'Ver tarea', command: () => this.router.navigate(['/tasks/view/' + this.card.taskId]).then(r => console.log('Navigate to view task'))},
      {label: 'Editar tarea', command: () => this.router.navigate(['/tasks/edit/' + this.card.taskId]).then(r => console.log('Navigate to edit task'))},
