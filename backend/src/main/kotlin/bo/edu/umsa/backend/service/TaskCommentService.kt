...
class TaskCommentService @Autowired constructor(
    ...
    fun getCommentById(commentId: Long): TaskCommentDto {
        ...
        val taskCommentEntity = taskCommentRepository.findByTaskCommentIdAndStatusIsTrue(commentId) ?: throw EtnException(
            HttpStatus.NOT_FOUND, "Error: Comment not found", "Comentario no encontrado"
        )
        // Validate that the user is the user who created the comment
        if (taskCommentEntity.userId.toLong() != userId) {
            throw EtnException(
                HttpStatus.FORBIDDEN,
                "Error: User is not the comment owner",
    ...
    fun createComment(newTaskCommentDto: NewTaskCommentDto) {
        ...
        if (newTaskCommentDto.comment.isEmpty()) {
            throw EtnException(
                HttpStatus.BAD_REQUEST, "Error: Comment is blank", "El comentario está en blanco"
            )
        }
        ...
        val taskEntity = taskRepository.findByTaskIdAndStatusIsTrue(newTaskCommentDto.taskId.toLong()) ?: throw EtnException(
        ...
        val taskCommentNumber = taskCommentRepository.findFirstByTaskIdOrderByCommentNumberDesc(newTaskCommentDto.taskId.toLong())
            ?.commentNumber?.plus(1) ?: 1
