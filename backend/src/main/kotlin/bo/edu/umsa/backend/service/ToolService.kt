...
class ToolService @Autowired constructor(
    ...
    fun getToolById(toolId: Long): ToolDto {
        ...
        val tool = toolRepository.findByToolIdAndStatusIsTrue(toolId) ?: throw EtnException(
            HttpStatus.NOT_FOUND, "Error: Tool not found", "Herramienta no encontrada"
        )
    ...
    fun createTool(newToolDto: NewToolDto) {
        ...
        if (newToolDto.toolCode.isEmpty() || newToolDto.toolName.isEmpty()) {
            throw EtnException(
                HttpStatus.BAD_REQUEST, "Error: At least one required field is blank", "Al menos un campo requerido está en blanco"
            )
        }
        // Validate the file photo id is valid
        val fileEntity = fileRepository.findByFileIdAndStatusIsTrue(newToolDto.filePhotoId.toLong()) ?: throw EtnException(
            HttpStatus.BAD_REQUEST, "Error: File photo is invalid", "La foto del archivo es inválida"
        )
        // Validate the file is an image
        if (!fileEntity.isPicture) {
            throw EtnException(
                HttpStatus.BAD_REQUEST, "Error: File photo is not an image", "La foto del archivo no es una imagen"
            )
        }
    ...
    fun updateTool(toolId: Long, newToolDto: NewToolDto) {
        ...
        val toolEntity = toolRepository.findByToolIdAndStatusIsTrue(toolId) ?: throw EtnException(
            HttpStatus.NOT_FOUND, "Error: Tool not found", "Herramienta no encontrada"
        )
    ...
    fun deleteTool(toolId: Long) {
        ...
        if (loanedToolRepository.findByToolIdAndStatusIsTrue(toolId) != null) {
            throw EtnException(
                HttpStatus.BAD_REQUEST, "Error: Tool is loaned", "La herramienta está prestada"
            )
        }
    ...
    fun assignToolToTask(toolId: Long, taskId: Long) {
        ...
        taskRepository.findByTaskIdAndStatusIsTrue(taskId) ?: throw EtnException(
            HttpStatus.NOT_FOUND, "Error: Task not found", "Tarea no encontrada"
        )
        // Validate the tool exists
        toolRepository.findByToolIdAndStatusIsTrue(toolId) ?: throw EtnException(
            HttpStatus.NOT_FOUND, "Error: Tool not found", "Herramienta no encontrada"
        )
        // Validate the user is assigned to the task
        taskAssigneeRepository.findByTaskIdAndUserIdAndStatusIsTrue(taskId, userId) ?: throw EtnException(
            HttpStatus.FORBIDDEN, "Error: User is not assigned to the task", "El usuario no está asignado a la tarea"
        )
